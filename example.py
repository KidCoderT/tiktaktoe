...
try:
    leaderboards = {
        r[0]: r[:2] + [int(r[2])] for r in list(reader(open("leaderboard.csv", "r")))
    }
except:
    leaderboards = {}
...
def update_leaderboard():
    lboards = sorted(leaderboards.values(), key=lambda x: int(x[2]), reverse=True)
...
async def gamemap(user, ctx):
    ...
    if user.id not in players:
        embed = discord.Embed(
            title=":space_invader: __**SPACE INVADERS**__ :space_invader:",
            description=to_send,
...
Score: **{players[user.id]['tscore']:,}**"""
    ...
    for i in range(1, 10):
        ...
        if stuff["enemy"] in players[user.id]["room"][i]:
            if i == 9:
                embed = discord.Embed(
                    title="__**GAME OVER**__ :space_invader:",
                    description=f"Your score: {players[user.id]['tscore']:,}",
                ...
                players[user.id]["room"][i + 1][
                    players[user.id]["room"][i].index(stuff["enemy"])
                ]
                == stuff["bullet"]
            ):
        ...
        if stuff["bullet"] in players[user.id]["room"][i]:
            ...
            if i == 1:
                ...
                players[user.id]["room"][i - 1][
                    players[user.id]["room"][i].index(stuff["bullet"])
                ]
                == stuff["enemy"]
            ):
...
bot = commands.Bot(
    ...
    owner_ids=[],
)  # Make sure to enable all intents in the developer portal.
...
async def hello(ctx):
    ...
    return await ctx.reply(
        "Hi! I'm the Retro Bot. Use **$help** for a list of my commands!",
        mention_author=True,
    )
...
async def refresh(ctx: commands.Context):
    ...
    with open("leaderboard.csv", "r") as board:
        ...
        if len(listcsv) > 10:
            listcsv = listcsv[:10]
        embed = discord.Embed(
            title=":space_invader: __**OLD LEADERBOARDS**__ :space_invader:",
...
async def lboard(ctx):
    ...
    with open("leaderboard.csv", "r") as board:
        ...
        if len(listcsv) > 10:
            listcsv = listcsv[:10]
        embed = discord.Embed(
            title=":space_invader: __**LEADERBOARDS**__ :space_invader:",
            description=f"```{tabulate(listcsv, headers=['Rank','Username','Score'])}```",
            colour=discord.Colour.gold(),
        )
...
async def jump(ctx):
    ...
    if ctx.message.author.id not in players:
        return await ctx.reply(
            f"You are not playing! Use **$play** to start playing.", mention_author=True
        )

    message = await ctx.fetch_message(players[ctx.message.author.id]["game"])
    return await message.reply(
        f"<@{ctx.message.author.id}>, here is your game!", mention_author=True
    )
...
async def play(ctx):
    ...
    if ctx.message.author.id in players:
        message = await ctx.fetch_message(players[ctx.message.author.id]["game"])
        return await message.reply(
            f"<@{ctx.message.author.id}>, you are already playing!\nHere is the running game! Use **$stop** to stop playing.",
            mention_author=True,
        )
...
async def stop(ctx):
    ...
    embed = discord.Embed(
        title=":space_invader: __**SPACE INVADERS**__ :space_invader:",
        description=f"Game stopped!\nYour Score: {players[ctx.message.author.id]['tscore']}",
        color=discord.Color.red(),
    )
